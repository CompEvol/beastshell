...
import org.renjin.Distributions;
import org.renjin.MathExt;
...
public class Binom {
  ...
  public static double rbinom(/*Session context,*/ double nin, double pp) {
    ...
    if (r >= Integer.MAX_VALUE) {
      return Distributions.qbinom(context.rng_unif_rand(), (int) r, pp, false, false);
    }
    /* else */
    n = (int) r;
    ...
    /*-------------------------- np = n*p >= 30 : ------------------- */
    while (true) {
      u = context.rng_unif_rand() * p4;
      v = context.rng_unif_rand();
      /* triangular region */
      if (u <= p1) {
        ix = (int) (xm - p1 * v + u);
