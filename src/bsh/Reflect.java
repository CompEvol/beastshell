...
final class Reflect {
	...
	static Object constructObject(Class clas, Object[] args) throws ReflectError, InvocationTargetException {
		...
		if (con == null) {
			// RRB: added for BEAST
			if (BEASTObject.class.isAssignableFrom(clas)) {
				types = new Class[0];
		...
		try {
			// RRB: added for BEAST
			if (BEASTObject.class.isAssignableFrom(clas)) {
				BEASTObject bo = (BEASTObject) con.newInstance();
				try {
					bo.initByName(args);
				} catch (Exception e) {
					throw new ReflectError("Something went wrong initialising BEASTObject: " + e);
				}
				return bo;
			}
